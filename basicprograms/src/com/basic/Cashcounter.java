...
class Cashcounter {
	...
	/**
	 * @param args
	 * driver function to implement cash counter
	 * always maintain grand total >0, deny invalid transactions.
	 */
	public static void main(String[] args) {
		...
		Cashcounter cc = new Cashcounter(people);
		Random rm = new Random();
		for (int i = 0; i < people; i++) {
			boolean action = true;    // True for Deposit, False for Withdraw
			int temp = rm.nextInt(100000) + 1;
			System.out.println("Enter whether Withdraw or Deposit for person "
					+ (i + 1));
			String purpose = t.next();//whether to deposit or withdraw
			...
			continue;
				...
				else {	System.out.println("Your old Bal. before " + purpose + " is "
						+ customer.getBal());
				customer.setBal(customer.getBal() - money);
				System.out.println("Your New Bal. after " + purpose + " is"
						+ customer.getBal());
			}
				}
			if (purpose.equalsIgnoreCase("deposit")) {
				System.out.println("Your old Bal. before " + purpose + " is "
						+ customer.getBal());
				customer.setBal(customer.getBal() + money);
				System.out.println("Your New Bal. after " + purpose + " is"
						+ customer.getBal());
			...
			if (balance >= 0) {
				...
				System.out
						.println("So Sorry. No more Transactions for the day");
				break;
			}
		}
...
/**
 * @author Ajit Shikalgar
 * Customer class containing info about customer
 *
 */
class Customer {
